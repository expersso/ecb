...
#Convert monthly to annual unemployment data due to missing annual data from ECB.
as.year <- function(x){as.numeric(floor(zoo::as.yearmon(x, "%Y-%m")))}

unemp <-  unemp %>%
  mutate(year = as.year(obstime)) %>%
  group_by(ref_area, year) %>%
  summarise(obsvalue = mean(obsvalue), obstime = as.character(unique(year))) %>% 
  select(ref_area, obstime, "unemp" = obsvalue)

wages <- wages %>% select(ref_area, obstime, "wage" = obsvalue)

df <- left_join(unemp, wages) %>% as.data.frame()
head(df)
```
